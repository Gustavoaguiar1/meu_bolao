...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  document.getElementById("valorApostaConfig")?.addEventListener("input", formatarCampoValorAposta);
...
async function salvarOuCriarBolao(event) {
  ...
  const valorAposta = lerValorApostaConfig();
  if (!timeCasa || !timeFora) { alert("Informe os dois times do bolão."); return; }
  if (!Number.isFinite(valorAposta) || valorAposta <= 0) { alert("Informe um valor de aposta válido."); return; }
  const payload = { time_casa: timeCasa, time_fora: timeFora, valor_aposta: valorAposta, chave_pix: getChavePix(), atualizado_em: new Date().toISOString() };
  let result;
...
function aplicarConfigBolaoNaTela() {
  ...
  const valorApostaConfig = document.getElementById("valorApostaConfig"); if (valorApostaConfig) valorApostaConfig.value = formatarValorInput(getValorAposta());
...
function lerValorApostaConfig(){ const input=document.getElementById("valorApostaConfig"); if(!input)return getValorAposta(); return valorMoedaParaNumero(input.value); }
function atualizarValorApostaNaTela(){ const valor=dinheiro(getValorAposta()); ["valorApostaTopo","valorApostaCard","valorApostaAviso","valorApostaFooter"].forEach(id=>{ const el=document.getElementById(id); if(el) el.textContent=valor; }); }
function formatarValorInput(valor){ return dinheiro(valor || 0); }
function formatarCampoValorAposta(event){ event.target.value = formatarValorInput(valorMoedaParaNumero(event.target.value)); }
function valorMoedaParaNumero(valor){ const centavos=somenteNumeros(valor); return centavos ? Number(centavos) / 100 : 0; }
